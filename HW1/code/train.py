...
train_dataset, val_dataset, _ = random_split(dataset=dataset, lengths=[500, 100, 11480])#[9664, 2416])
# train_dataset, val_dataset = random_split(dataset=dataset, lengths=[9664, 2416])  # 训练集和验证集划分 4:1
train_dataloader = DataLoader(dataset=train_dataset, batch_size=1, shuffle=True, num_workers=0)  # dataloader batch_size设置的小是因为显存不够
val_dataloader = DataLoader(dataset=val_dataset, batch_size=1, shuffle=True, num_workers=0)
